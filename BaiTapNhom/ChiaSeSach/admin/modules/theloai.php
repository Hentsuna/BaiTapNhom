...
if(isset($_GET['dele'])){
    ...
    if($id === '0'){
        ...
        $check = mysqli_num_rows(mysqli_query($conn,"SELECT * from categories where ma_the_loai = $getidtheloai"));
        if($check > 0)     echo '<script>alert("Bạn không thể xóa vì vẫn còn sách thuộc thể loại này")</script>';
        else mysqli_query($conn, "delete from categories where ma_the_loai = $id");
    }
    echo '<script>window.location="'.$domain.'/admin/?chucnang=theloai";</script>';
}
if(isset($_GET['edit'])){
...
<table style="width: 100%; margin: 5px;">
    ...
    <tr style="background: #fff">
        ...
        <td style="color: #115095; font-weight: 500">Số lượng sách</td>
        <td style="color: #115095; font-weight: 500">Hiển thị ở menu</td>
    ...
    <?php
        ...
        if(mysqli_num_rows($data) > 0){
            ...
            while ($getdata = mysqli_fetch_assoc($data)){
                $getidthloai = $getdata['ma_the_loai'];
                $getsoluong = mysqli_num_rows(mysqli_query($conn, "Select * from books where ma_the_loai = $getidthloai"));
